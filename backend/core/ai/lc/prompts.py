...
4. goals 是主结果，必须按原话顺序列出每个子目标。
5. 每个 goal 填 id/action/target/value/completion/required_ops/missing；action 是自由语义摘要，不用于代码匹配。
6. required_ops 只能从执行器能力中选择：activate|ocr_click|type|key|wait|wait_text|schedule|find_image_click|color_click|loop|if_text|try_catch。
7. missing 归属具体 goal；不要用预设任务类型推导缺失信息。
8. 不要输出 Flow JSON。
"""
