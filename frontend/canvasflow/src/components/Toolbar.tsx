...
export default function Toolbar({
  ...
  const applyPluginUiClass = (enabled: boolean, opacity = pluginOpacity) => {
    ...
    document.documentElement.style.setProperty('--plugin-fill', enabled ? `${Math.round(op * 70)}%` : '100%');
  ...
  const persistPluginPrefs = (state: { enabled: boolean; opacity: number; click_through: boolean }) => {
    try {
  ...
  const syncPluginMode = async (patch: { enabled?: boolean; opacity?: number; click_through?: boolean }) => {
          ...
          <div className="flex items-center gap-0.5 bg-black/5 dark:bg-white/5 p-1 rounded-2xl border border-black/10 dark:border-white/10">
            ...
            {onToggleDebug && (
              <Button
                ...
                style={debugMode ? { color: '#d97706', backgroundColor: 'rgba(245, 158, 11, 0.12)' } : undefined}>
        ...
        <div className="relative z-10 h-full flex items-center justify-between px-2">
          ...
          <div className="flex items-center gap-1 shrink-0 z-10">
            <Button
              ...
              {annDot ? <span className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-amber-500" /> : null}
            </Button>
