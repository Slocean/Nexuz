  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-center gap-3 min-w-0 shrink-0 pl-4">
        ...
        >
          <Workflow style={{ color: colors.primary }} className="w-5 h-5" />
        </div>
        <div className="flex items-center gap-2 min-w-0">
          <Input
            value={workflowName}
            onChange={(e) => setWorkflowName(e.target.value)}
            className="h-8 max-w-[160px] border-transparent bg-transparent font-display font-semibold text-base hover:border-white/20 focus:border-blue-500 px-1"
            placeholder="Untitled Flow"
          />
          <Badge variant="secondary" className="font-mono uppercase tracking-wide shrink-0">
            Nexuz
          </Badge>
        </div>
      </div>
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
        <div className="flex items-center gap-1 bg-black/5 dark:bg-white/5 p-1 rounded-2xl border border-white/5 whitespace-nowrap">
          <Button
            size="sm"
          ...
          {execStatus === 'paused' ? (
            <Button variant="ghost" size="sm" onClick={onResume} title="继续">
              ...
              <span className="hidden xl:inline">继续</span>
            </Button>
          ) : (
            <Button
              variant="ghost"
          ...
          {onStep && (
            ...
            >
              ...
              <span className="hidden xl:inline">单步</span>
            </Button>
          )}
          ...
          <Button variant="ghost" size="sm" onClick={handleSave} title="保存">
            {isSaved ? (
              <>
                ...
                <span className="text-emerald-500 hidden sm:inline">Saved</span>
              </>
            ) : (
              <>
          ...
          {onToggleRecord && (
            <Button
              ...
              title={recording ? '停止录制' : '录制'}
            >
              <CircleDot className={`w-3.5 h-3.5 ${recording ? 'animate-pulse' : ''}`} />
              <span className="hidden lg:inline">{recording ? '停止' : '录制'}</span>
            </Button>
          )}
      ...
      <div className="relative z-10 ml-auto flex items-center gap-1 shrink-0 pr-1.5">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="h-9 w-9" title="主题">
        ...
        <Button
          ...
          style={
            isAssistantOpen
              ...
              : { borderColor: colors.primary + '30', color: colors.primary }
          }
        >
